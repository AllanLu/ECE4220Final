/* 	Name       : 	Final.cpp
	Author     : 	Yiwei Lu and Brent Schultz
...
//for time delays
int make_periodic(struct periodinfo *info){   
...
void wait_period(struct periodinfo *info){
	int ret;
	unsigned long long missed;
	ret = read(info->timer_fd, &missed, sizeof(missed));
	if(ret == -1){
		printf("WAIT PERIOD ERROR\n");
		return;
	}
	info->missed += missed;
	return;
